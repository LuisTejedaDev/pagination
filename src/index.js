...
export default () => {
    ...
    const Item = ({id, page, titulo, descripcion}) => {
        return(
            <View style={{height: 150, alignSelf: 'stretch', backgroundColor: '#f9f9f9', borderWidth: 2, borderColor: '#dadada', paddingHorizontal: 12, paddingBottom: 12, marginTop: id === 1 || id === (((page - 1) * 10) + 1) ? 12 : 0, marginBottom: id === (page * 10) ? 87 : 12}}>
                <View style={{height: 50, alignSelf: 'stretch', justifyContent: 'center', alignItems: 'center'}}>
                    <Text style={{fontWeight: 'bold', color: '#383838', fontSize: 16}}>{`${id} ${titulo}`}</Text>
                </View>
                <View style={{flex: 1}}>
                    <Text style={{fontSize: 14, fontWeight: '300', color: '#adadad'}}>{descripcion.length > 180 ? `${descripcion.substring(0,180)}...` : descripcion}</Text>
                </View>
            </View>
        )
    }
    ...
    return(
        <>
            ...
            <Pagination data={data} Item={Item}/>
        </>
    )
}